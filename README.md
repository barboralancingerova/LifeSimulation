...
> Zápočtový program, [DOPLNIT: předmět], [DOPLNIT: semestr / rok]
> Autor: [DOPLNIT]
> Technologie: C# / .NET [DOPLNIT verzi], Blazor Server (InteractiveServer), bez externích knihoven
> Vývoj: tablet Android, Termux [DOPLNIT: jak přesně běží dotnet]

![Hlavní obrazovka simulace](docs/img/screenshot-main.png)
...
Uživatel simulaci ovládá ve webovém prohlížeči: spouští, krokuje, mění během běhu
intenzitu slunce a energetické náklady, a sleduje populační křivky a energii systému
...
```bash
git clone [DOPLNIT: URL]
cd GameOfLife
dotnet run
```

Aplikace poběží na `http://localhost:[DOPLNIT port]`; simulace je na kořenové stránce `/`.
Požadavek: .NET SDK [DOPLNIT].
...
│   └── img/                  # screenshoty
├── [DOPLNIT: složka jádra]   # Config, Energy, Cell, Grid, Organism, Producer,
│                             # Herbivore, Predator, Genome, NeuralNetwork,
│                             # Statistics, RuntimeSettings
├── [DOPLNIT: složka UI]      # SimulationPage.razor (+css), Chart.razor (+css),
│                             # InitControls.razor, RuntimeControls.razor (nepoužité)
├── wwwroot/img/              # ikony organismů a tlačítek
└── Program.cs                # [DOPLNIT]
```

Rozsah: 1 637 řádků, z toho ~1 086 simulační jádro (C#) a ~551 rozhraní (Razor + CSS).
...
zvýhodňuje pohyb „dopředu“; geny `StepEnergyCost` a `MovementEnergyCost` se dědí, ale
chování neovlivňují; absolutní σ mutace je pro velké geny prakticky nulová; globální
